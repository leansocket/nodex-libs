(function () {
   ...
   let fs = require('fs');
   let debug = require('debug')('simple-git');
   let responses = require('./responses');

   let NOOP = function () { };

   let exists = function (path, type) {
      let exists_func = function (path, isFile, isDirectory) {
         try {
            var matches = false;
      ...
      if (!type) {
         return exists_func(path, true, true);
      }

      return exists_func(path, type & 1, type & 2);
   };
   ...
   let defferred = function deferred() {
      var d = {};
